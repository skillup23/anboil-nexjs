import Link from 'next/link';

function HeaderUpInfo() {
  return (
    <ul>
      <li>
        <Link href="/">
          <a className="font__base">
            <svg
              width="20"
              height="22"
              viewBox="0 0 20 22"
              fill="none"
              xmlns="http://www.w3.org/2000/svg">
              <path id="path1" d="M14.5 8.39795L5.5 3.20795M1.27 5.95795L10 11.0079L18.73 5.95795M10 21.0779V10.9979M19 14.9979V6.99795C18.9996 6.64722 18.9071 6.30276 18.7315 5.99911C18.556 5.69546 18.3037 5.44331 18 5.26795L11 1.26795C10.696 1.09241 10.3511 1 10 1C9.64893 1 9.30404 1.09241 9 1.26795L2 5.26795C1.69626 5.44331 1.44398 5.69546 1.26846 5.99911C1.09294 6.30276 1.00036 6.64722 1 6.99795V14.9979C1.00036 15.3487 1.09294 15.6931 1.26846 15.9968C1.44398 16.3004 1.69626 16.5526 2 16.7279L9 20.7279C9.30404 20.9035 9.64893 20.9959 10 20.9959C10.3511 20.9959 10.696 20.9035 11 20.7279L18 16.7279C18.3037 16.5526 18.556 16.3004 18.7315 15.9968C18.9071 15.6931 18.9996 15.3487 19 14.9979Z"
                stroke="#F8F8F8"
                strokeWidth="1.5"
                strokeLinecap="round"
                strokeLinejoin="round">
              </path>
            </svg>
            Рассчитать
            <br />
            доставку ГСМ
          </a>
        </Link>
      </li>
      <li>
        <Link href="/">
          <a className="font__base">
            <svg
              width="20"
              height="22"
              viewBox="0 0 20 22"
              fill="none"
              xmlns="http://www.w3.org/2000/svg">
              <path id="path1" d="M14.5 8.39795L5.5 3.20795M1.27 5.95795L10 11.0079L18.73 5.95795M10 21.0779V10.9979M19 14.9979V6.99795C18.9996 6.64722 18.9071 6.30276 18.7315 5.99911C18.556 5.69546 18.3037 5.44331 18 5.26795L11 1.26795C10.696 1.09241 10.3511 1 10 1C9.64893 1 9.30404 1.09241 9 1.26795L2 5.26795C1.69626 5.44331 1.44398 5.69546 1.26846 5.99911C1.09294 6.30276 1.00036 6.64722 1 6.99795V14.9979C1.00036 15.3487 1.09294 15.6931 1.26846 15.9968C1.44398 16.3004 1.69626 16.5526 2 16.7279L9 20.7279C9.30404 20.9035 9.64893 20.9959 10 20.9959C10.3511 20.9959 10.696 20.9035 11 20.7279L18 16.7279C18.3037 16.5526 18.556 16.3004 18.7315 15.9968C18.9071 15.6931 18.9996 15.3487 19 14.9979Z"
                stroke="#F8F8F8"
                strokeWidth="1.5"
                strokeLinecap="round"
                strokeLinejoin="round">
              </path>
            </svg>
            Рассчитать
            <br />
            хранение ГСМ
          </a>
        </Link>
      </li>
      <li>
        <Link href="/">
          <a className="font__base">
            <svg
              width="21"
              height="21"
              viewBox="0 0 21 21"
              fill="none"
              xmlns="http://www.w3.org/2000/svg">
              <path id="path2" d="M4.92188 8.32617H3.30098C3.5748 4.59375 6.69892 1.64062 10.5 1.64062C13.5078 1.64062 16.2296 3.53514 17.2727 6.35492C17.4299 6.7798 17.9017 6.99681 18.3267 6.83968C18.7516 6.68247 18.9686 6.21063 18.8114 5.78571C18.1897 4.10505 17.0852 2.66688 15.6174 1.62664C14.1158 0.562488 12.3462 0 10.5 0C8.1336 0 5.90879 0.921539 4.23548 2.59485C2.58398 4.24635 1.66544 6.43511 1.64173 8.76697C0.661377 9.33503 0 10.3951 0 11.6074V14.3965C0 16.2058 1.47197 17.6777 3.28125 17.6777H4.92188C5.37493 17.6777 5.74219 17.3105 5.74219 16.8574V9.14648C5.74219 8.69343 5.37493 8.32617 4.92188 8.32617ZM4.10156 16.0371H3.28125C2.37661 16.0371 1.64062 15.3011 1.64062 14.3965V11.6074C1.64062 10.7028 2.37661 9.9668 3.28125 9.9668H4.10156V16.0371Z"
                fill="#F8F8F8">
              </path>
              <path id="path2" d="M17.7188 8.32617H16.0781C15.6251 8.32617 15.2578 8.69343 15.2578 9.14648V17.7188C15.2578 18.1711 14.8898 18.5391 14.4375 18.5391H11.9355V18.5651C11.6557 18.0605 11.1179 17.7188 10.5 17.7188C9.59392 17.7188 8.85938 18.4533 8.85938 19.3594C8.85938 20.2655 9.59392 21 10.5 21C11.1179 21 11.6557 20.6583 11.9355 20.1537V20.1797H14.4375C15.7945 20.1797 16.8984 19.0757 16.8984 17.7188V17.6777H17.7188C19.528 17.6777 21 16.2058 21 14.3965V11.6074C21 9.79814 19.528 8.32617 17.7188 8.32617ZM19.3594 14.3965C19.3594 15.3011 18.6234 16.0371 17.7188 16.0371H16.8984V9.9668H17.7188C18.6234 9.9668 19.3594 10.7028 19.3594 11.6074V14.3965Z"
                fill="#F8F8F8">
              </path>
            </svg>
            Получить
            <br />
            консультацию
          </a>
        </Link>
      </li>

      <style jsx>{`
        ul {
          display: flex;
          flex-wrap: wrap;
          align-items: center;
          margin-block-start: 0;
          margin-block-end: 0;
          padding-inline-start: 0;
        }
        
        li {
          display: block;
        }
        
        li:not(:last-child) {
          margin-right: 72px;
        }
        
        a {
          display: flex;
          justify-content: space-between;
          align-items: flex-start;
          color: white;
          text-decoration: none;
          text-transform: uppercase;
          font-size: 10px;
          line-height: 1.5;
          font-weight: 500;
          transition: .2s ease;
        }
        
        a:hover {
          color: #EE0A0E;
        }
        
        a svg path {
          transition: .2s ease;
          margin-right: 12px;
        }
        
        a:hover svg #path1 {
          stroke: #EE0A0E;
        }
        
        a:hover svg #path2 {
          fill: #EE0A0E;
        }
        
        svg {
          margin-right: 12px;
        }
      `}</style>
    </ul>
  );
}

export default HeaderUpInfo;